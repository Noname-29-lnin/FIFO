...
void Vtb_fifo::eval_end_step() {
    VL_DEBUG_IF(VL_DBG_MSGF("+eval_end_step Vtb_fifo::eval_end_step\n"); );
#ifdef VM_TRACE
    // Tracing
...
VL_ATTR_COLD void Vtb_fifo::trace(VerilatedVcdC* tfp, int levels, int options) {
    if (tfp->isOpen()) {
        vl_fatal(__FILE__, __LINE__, __FILE__,"'Vtb_fifo::trace()' shall not be called after 'VerilatedVcdC::open()'.");
    }
    if (false && levels && options) {}  // Prevent unused
    tfp->spTrace()->addModel(this);
    tfp->spTrace()->addInitCb(&trace_init, &(vlSymsp->TOP));
    Vtb_fifo___024root__trace_register(&(vlSymsp->TOP), tfp->spTrace());
}
